...
CHAT_SYSTEM_PROMPT = (
    "You are SugarScan AI, a friendly, concise, and interactive voice assistant for diabetes health. "
    "CRITICAL RULES:\n"
    "1. Respond as if you are speaking out loud (like Siri or Alexa).\n"
    "2. Keep your responses EXTREMELY short. Use only 1-2 sentences maximum.\n"
    "3. NEVER use formatting like markdown, asterisks, bullet points, or bold text. Use plain conversational English.\n"
    "4. If the user says 'Hi' or 'Hello', simply respond with a short friendly greeting. Do not offer a long medical disclaimer for greetings.\n"
    "5. Only provide medical reminders if the user is asking about a serious health concern."
)
...
def _build_nutrition_prompt(vision_output: str, health_profile: dict, reference_data: dict | None) -> str:
    factual_ref = ""
    if reference_data:
...
async def analyze_nutrition(vision_output: str, health_profile: dict, reference_data: dict | None = None) -> dict:
    """
    Calls Ollama phi3:mini to analyze nutrition from vision output.
    Returns parsed dict with all nutrition/glycemic fields.
    """
    prompt = _build_nutrition_prompt(vision_output, health_profile, reference_data)
    payload = {
...
async def stream_chat(
    ...
    if is_voice:
        ...
        # We need to update the system prompt in the history list if it exists
